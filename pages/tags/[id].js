...
import tagsPageInitialPathsTqC5Resource from '../../resources/tags-page-initial-paths-tq_c5'
import tagsPageInitialPropsTqQjResource from '../../resources/tags-page-initial-props-tq_qj'
...
export async function getStaticPaths() {
  try {
    const response = await tagsPageInitialPathsTqC5Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await tagsPageInitialPropsTqQjResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
