...
import apartmentsPageInitialPathsTqKbResource from '../../resources/apartments-page-initial-paths-tq_kb'
import apartmentsPageInitialPropsTqVzResource from '../../resources/apartments-page-initial-props-tq_vz'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqKbResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqVzResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
