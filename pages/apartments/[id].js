...
import apartmentsPageInitialPathsTqR1Resource from '../../resources/apartments-page-initial-paths-tq_r1'
import apartmentsPageInitialPropsTqLbResource from '../../resources/apartments-page-initial-props-tq_lb'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqR1Resource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqLbResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
