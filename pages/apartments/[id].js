...
import apartmentsPageInitialPathsTqXuResource from '../../resources/apartments-page-initial-paths-tq_xu'
import apartmentsPageInitialPropsTqVeResource from '../../resources/apartments-page-initial-props-tq_ve'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqXuResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqVeResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
