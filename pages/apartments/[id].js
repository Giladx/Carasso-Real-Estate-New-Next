...
import apartmentsPageInitialPathsTqTyResource from '../../resources/apartments-page-initial-paths-tq_ty'
import apartmentsPageInitialPropsTqCRResource from '../../resources/apartments-page-initial-props-tq_c-r'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqTyResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqCRResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
