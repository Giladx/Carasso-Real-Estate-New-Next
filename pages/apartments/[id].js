...
import apartmentsPageInitialPathsTqNlResource from '../../resources/apartments-page-initial-paths-tq_nl'
import apartmentsPageInitialPropsTqFiResource from '../../resources/apartments-page-initial-props-tq_fi'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqNlResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqFiResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
