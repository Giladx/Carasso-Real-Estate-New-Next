...
import apartmentsPageInitialPathsTqKZResource from '../../resources/apartments-page-initial-paths-tq_k-z'
import apartmentsPageInitialPropsTqVyResource from '../../resources/apartments-page-initial-props-tq_vy'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqKZResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqVyResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
