...
import apartmentsPageInitialPathsTqWLResource from '../../resources/apartments-page-initial-paths-tq_w-l'
import apartmentsPageInitialPropsTqR5Resource from '../../resources/apartments-page-initial-props-tq_r5'
...
export async function getStaticPaths() {
  try {
    const response = await apartmentsPageInitialPathsTqWLResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await apartmentsPageInitialPropsTqR5Resource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
