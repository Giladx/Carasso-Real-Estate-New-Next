...
import tabsPageInitialPropsTq0eResource from '../../resources/tabs-page-initial-props-tq_0e'
import tabsPageInitialPathsTqXdResource from '../../resources/tabs-page-initial-paths-tq_xd'
...
export async function getStaticProps(context) {
  try {
    const response = await tabsPageInitialPropsTq0eResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await tabsPageInitialPathsTqXdResource({})
    return {
