...
import shopsPageInitialPropsTqLiResource from '../../resources/shops-page-initial-props-tq_li'
import shopsPageInitialPathsTqIGResource from '../../resources/shops-page-initial-paths-tq_i-g'
...
export async function getStaticProps(context) {
  try {
    const response = await shopsPageInitialPropsTqLiResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await shopsPageInitialPathsTqIGResource({})
    return {
