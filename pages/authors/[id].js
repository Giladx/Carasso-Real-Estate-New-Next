...
import authorsPageInitialPathsTqUkResource from '../../resources/authors-page-initial-paths-tq_uk'
import authorsPageInitialPropsTqWbResource from '../../resources/authors-page-initial-props-tq_wb'
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsTqUkResource({})
    return {
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialPropsTqWbResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
