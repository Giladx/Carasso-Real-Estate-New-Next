...
import authorsPageInitialPropsTqWqResource from '../../resources/authors-page-initial-props-tq_wq'
import authorsPageInitialPathsTqO3Resource from '../../resources/authors-page-initial-paths-tq_o3'
...
export async function getStaticProps(context) {
  try {
    const response = await authorsPageInitialPropsTqWqResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await authorsPageInitialPathsTqO3Resource({})
    return {
