...
import brandsPageInitialPropsTqXSResource from '../../resources/brands-page-initial-props-tq_x-s'
import brandsPageInitialPathsTqQmResource from '../../resources/brands-page-initial-paths-tq_qm'
...
export async function getStaticProps(context) {
  try {
    const response = await brandsPageInitialPropsTqXSResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await brandsPageInitialPathsTqQmResource({})
    return {
